...
using FunctionalCSharp.Books.FunctionalProgrammingInCSharp.Part3_FunctionalDesigns.Chap8_FunctionalErrorHandling.Controllers.BookTransfers.Errors;
...
public partial class Chapter06BookTransferController : Chapter06ControllerBase
{
    ...
    private Either<Error, BookTransfer> ValidateBic1(BookTransfer bookTransfer)
    {
        var regex = new Regex("[A-Z]{11}");
        return  regex.IsMatch(bookTransfer.Bic) ? bookTransfer : Chap06BaseErrors.InvalidBic;
    }

    private Validation<BookTransfer> ValidateBic2(BookTransfer bookTransfer)
    {
        var regex = new Regex("[A-Z]{11}");
        return regex.IsMatch(bookTransfer.Bic) ? bookTransfer : Invalid(Chap06BaseErrors.InvalidBic);
    }
