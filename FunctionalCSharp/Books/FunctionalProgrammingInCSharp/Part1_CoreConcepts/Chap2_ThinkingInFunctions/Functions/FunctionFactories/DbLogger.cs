...
namespace FunctionalCSharp.Books.FunctionalProgrammingInCSharp.Part1_CoreConcepts.Chap2_ThinkingInFunctions.Functions.FunctionFactories
{
    public class DbLogger
    {
        ...
        public int LogFunctional(LogMessage message) 
            => ConnectUsingFunctional(DbConnectionString, conn => conn.Execute("sp_create_log", message, commandType: StoredProcedure));
        ...
        public IEnumerable<LogMessage> GetLogsFunctional(DateTime since) 
            => ConnectUsingFunctional(DbConnectionString, conn => conn.Query<LogMessage>(SelectFromLogsSinceSql, new{since}));

        

        public int MyExecutor(LogMessage message) 
            => ConnectUsingFunctional(DbConnectionString, conn => conn.MyExecute(5, 10));
